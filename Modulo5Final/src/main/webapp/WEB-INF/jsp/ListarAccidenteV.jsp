...
<table id="example" class="row-border" style="width:40%">
	...
	<c:forEach items="${listaacc}" var="accidentes">
	<tbody>
	<tr>
		<td>${accidentes.getRut_Id()}</td>
		<td>${accidentes.getFecha()}</td>
		<td>${accidentes.getSuceso()}</td>
		<td>${accidentes.getLugar()}</td>
		<td>${accidentes.getDias_Perdidos()}</td>
		<td>${accidentes.getNum_Trab()}</td>
...
<br/>
<a href="AgregarClienteC">Agregar cliente</a>
