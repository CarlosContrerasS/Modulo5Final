...
<table id="example" class="row-border" style="width:40%">
	...
	<tr>
		<th> Rut Cliente</th>
		<th>N° Identificacion del accidente</th>
		<th>Fecha</th>
		<th>Hora</th>
		<th>Suceso</th>
		<th>Lugar</th>
	...
	<c:forEach items="${laccidente}" var="accidentes">
	<tbody>
	<tr>
		<td>${accidentes.getRutfk().getRut()}</td>
		<td>${accidentes.getIdaccidente()}</td>
		<td>${accidentes.getFecha()}</td>
		<td>${accidentes.getHora()}</td>
		<td>${accidentes.getSuceso()}</td>
		<td>${accidentes.getLugar()}</td>
		<td>${accidentes.getDiasperdidos()}</td>
		<td>${accidentes.getNumtrab()}</td>
